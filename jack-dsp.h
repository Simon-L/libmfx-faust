...
class jackaudio_midi : public jackaudio, public jack_midi {
    protected:
        ...
        virtual void processAudio(jack_nframes_t nframes)
        {
            ...
            // printf("-> written: %d\n", written);
            // memcpy(memory, fOutChannel[0], nframes * sizeof(float));
            // printf("sizeof buffer one channel: %u\n", sizeof(fOutChannel[0]));
            // printf("sizeof buffer alls channels: %u\n", sizeof(fOutChannel));
            // printf("sizeof float: %u\n", sizeof(float));
            // float tmp[nframes];
            // printf("-> tmp: %d %zu\n", nframes, sizeof(tmp));
            // for (size_t i = 0; i < nframes; i++) tmp[i] = fOutChannel[0][i];
            // size_t written = ringbuffer_write(rb, (void*)fOutChannel[0], sizeof(float) * nframes);
            // printf("sizeof big truc %d shifted2: %d divided: %d\n", sizeof(rb_compute_buffers), sizeof(rb_compute_buffers) << 2, (sizeof(rb_compute_buffers) << 2) / sizeof(rb_compute_buffers) );
            // printf("sizeof big truc %d shifted3: %d divided: %d\n", sizeof(rb_compute_buffers), sizeof(rb_compute_buffers) << 3, (sizeof(rb_compute_buffers) << 3) / sizeof(rb_compute_buffers) );
            // printf("sizeof big truc %d shifted4: %d divided: %d\n", sizeof(rb_compute_buffers), sizeof(rb_compute_buffers) << 4, (sizeof(rb_compute_buffers) << 4) / sizeof(rb_compute_buffers) );
            // printf("-> written %d\n", written);
            //   printf("%d:%f\n", i, fOutChannel[0][i]);
            // }
            // size_t available = ringbuffer_write_space(rb);
            // printf("available: %d size: %d\n", available, nframes * sizeof(float));
            // size_t available = ringbuffer_write_space(rb);
            // if (available % 4 == 0)
            // {
            //     size_t written = ringbuffer_write(rb, (void *)fOutChannel[0], sizeof(float) * nframes);
            //     if (written) printf("computed: %d %f %f %f %f %f %f %f %f\n", fOutChannel[0][0], fOutChannel[0][1], fOutChannel[0][2], fOutChannel[0][3], fOutChannel[0][4], fOutChannel[0][5], fOutChannel[0][6], fOutChannel[0][7]);
            //     printf("-> nframes: %d wrote: %d\n", nframes, written);
            // } else {
            //     printf("Nope2 %d\n", available);
            // }
